...
這個功能是通用型的，**不需要照特定欄位設計**，網站會直接照你 Google 表單目前的問題順序顯示，不管你表單裡有幾題、問題叫什麼名字都可以。**如果表單裡有「上傳檔案」題型，回覆裡的網址會自動被辨識成「📎 查看附件」按鈕**，不會顯示成一長串網址文字，這個是自動判斷的，不需要額外設定。
...
- 表單：`https://forms.gle/edno8MoVbJxoMRsH9`
...
- 如果表單裡有「上傳檔案」題型，回覆會自動顯示成「📎 查看附件」按鈕，用法跟公告事項的附件功能一樣

⚠️ **附件權限提醒**：跟公告事項的附件一樣，Google 表單上傳的檔案會存進表單擁有者的雲端硬碟資料夾，記得把那個資料夾的共用權限改成「知道連結的任何人」可檢視，同仁才打得開附件（詳細步驟見第六步的說明）。
...
同一台裝置只要輸入正確通行碼一次，之後在同一個瀏覽器都不用再輸入（除非清除瀏覽器資料）。登入成功後，畫面標題旁會顯示「○○○ 您好」，方便確認是哪位同仁登入的。

### 1. 建立存放通行碼的 Google 試算表

建立一個新的 Google 試算表，**A 欄放通行碼，B 欄放對應的姓名**，可以有很多列，每位同仁都可以有自己專屬的通行碼：
...
第一列可以放表頭（例如 `通行碼`、`姓名`）也可以不放，網站會自動略過看起來不像資料的列。**不需要**跟同仁名單、通訊錄等其他資料混在同一份試算表裡——這份試算表因為要發布成「任何人都能讀取」的公開網址，只放通行碼跟姓名就好，避免不小心把其他個資（電話、地址等）也一起公開發布出去。

### 2. 發布成網路可讀的 CSV

1. **檔案 → 共用 → 發布到網路**
2. 選擇正確的工作表，格式選 **逗號分隔值檔案 (.csv)**，點發布
3. 複製產生的網址

### 3. 把網址填回網站

打開 `auth.js`，找到 `STAFF_AUTH_CSV_URL`，換成剛剛複製的網址，存檔上傳。

### 之後怎麼新增/修改同仁的通行碼？

直接到那份試算表裡新增一列（新同仁）、或修改某一列的通行碼／姓名就好，不用改網站程式碼。跟公告事項、儲位查詢一樣，Google 發布快取大約 1-5 分鐘生效。

**提醒同仁**：某位同仁換了通行碼後，之前已經登入過的裝置不會被登出（除非清除瀏覽器資料），新密碼只影響「還沒登入過的人」或「已清除資料重新登入的人」。
...
| 4 | 附件檔案 | 上傳檔案（選填），這一題**一定要放在最後一題** |

表單會自動記錄「時間戳記」放在最前面，不用另外加欄位。

**置頂功能說明**：填表單時選「是」，這則公告會固定排在清單最上方（無論時間新舊），並顯示「📌 置頂」標記；選「否」則照時間排序。同時有多筆置頂時，置頂公告之間仍依時間新到舊排列。

**附件功能說明**：填表單時上傳的檔案，會顯示在公告卡片下方一個「📎 查看附件」的按鈕，點下去會開啟 Google Drive 檢視該檔案。如果同一則公告上傳多個檔案，會分別顯示「📎 附件 1」「📎 附件 2」……

⚠️ **附件權限重要提醒**：Google 表單的「上傳檔案」題型，會把檔案存進**表單擁有者**的 Google 雲端硬碟裡一個自動建立的資料夾（通常叫「表單名稱 (檔案回應)」）。這個資料夾預設**不是公開的**，同仁點「查看附件」可能會看到「沒有存取權限」。要解決這個問題：

1. 到 Google 雲端硬碟，找到那個自動建立的資料夾
2. 對資料夾按右鍵 → **共用** → 一般存取權改成 **「知道連結的任何人」**，權限選 **檢視者**
3. 這樣同仁點附件連結才能正常打開
